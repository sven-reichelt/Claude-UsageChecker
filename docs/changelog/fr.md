...
### Ajouté
- **Un guide d'utilisation, dans les neuf langues.** Il parcourt tout ce que fait
  l'application, fenêtre par fenêtre et avec une image de chacune : installation,
  connexion, icône, menu, fenêtre de détails, avertissements, chaque paramètre,
  mises à jour, désinstallation, et quoi faire quand quelque chose ne marche pas.
  Les images sont dessinées dans chaque langue à partir de chiffres d'exemple :
  aucune ne contient le compte de qui que ce soit.
- **À propos de Claude UsageChecker → Ouvrir le guide d'utilisation** l'ouvre dans
  la langue choisie pour l'interface.

## [1.0.1] – 2026-09-17

### Ajouté
- **Mises à jour automatiques.** Une nouvelle version trouvée au démarrage est
  installée sans question, et l'application redémarre dessus – le seul moment où
  personne n'est en pleine utilisation. Activées par défaut, dans **Paramètres →
  Mises à jour automatiques** ; désactivées, le démarrage signale qu'une nouvelle
  version existe, comme avant. Rien n'est installé tant qu'un avertissement
  d'utilisation attend une confirmation, et seule l'application installée se
  remplace elle-même.
- **Une vérification des mises à jour toutes les deux heures.** Elle signale une
  nouvelle version une seule fois, avec **Mettre à jour maintenant** ou **Me le
  rappeler demain**, et n'installe jamais d'elle-même. Elle suit le canal choisi,
  préversions comprises.

### Modifié
- **Des interrupteurs au lieu de cases à cocher dans les paramètres**, dessinés
  comme sous iOS : gris quand ils sont désactivés, de la couleur de l'application
  quand ils sont activés, avec un bouton qui glisse.
...
### Ajouté
- **Le paquet macOS est signé et notarisé.** Il portait jusqu'ici une signature
  ad hoc, sans valeur pour quiconque hormis la machine qui l'avait produite :
  macOS refusait de l'ouvrir, et y parvenir demandait de contredire le système
  à la main. Il porte désormais un Developer ID et la notarisation d'Apple,
  agrafée dans le paquet afin qu'elle vaille aussi hors ligne. Il s'ouvre d'un
  double-clic, comme tout le reste.
- **Remplacement automatique sur macOS.** Le bouton qui met à jour la version
  Windows depuis la 0.4.0 fait maintenant de même ici. La différence tient à ce
  qui est remplacé — un paquet entier plutôt qu'un fichier — et à ce qui est
  vérifié auparavant : outre la somme SHA-256 publiée, la signature de la
  version téléchargée, avec la question même que poserait Gatekeeper. Ce que
  macOS refuserait d'exécuter n'est pas installé non plus.
