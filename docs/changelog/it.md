...
### Aggiunto
- **Una guida all'uso, in tutte e nove le lingue.** Percorre tutto ciò che
  l'applicazione fa, finestra per finestra e con un'immagine di ciascuna:
  installazione, accesso, icona, menu, finestra dei dettagli, avvisi, ogni
  impostazione, aggiornamenti, disinstallazione e cosa fare quando qualcosa non
  funziona. Le immagini sono disegnate in ogni lingua a partire da valori di
  esempio, quindi nessuna contiene l'account di qualcuno.
- **Informazioni su Claude UsageChecker → Apri la guida** la apre nella lingua
  impostata per l'interfaccia.

## [1.0.1] – 2026-09-17

### Aggiunto
- **Aggiornamenti automatici.** Una nuova versione trovata all'avvio viene
  installata senza chiedere e l'applicazione si riavvia con essa: l'unico momento
  in cui nessuno è nel mezzo di qualcosa. Attivi per impostazione predefinita, in
  **Impostazioni → Aggiornamenti automatici**; se disattivati, l'avvio segnala che
  c'è una nuova versione, come prima. Nulla viene installato mentre un avviso di
  utilizzo attende conferma, e solo l'applicazione installata si sostituisce da
  sola.
- **Un controllo degli aggiornamenti ogni due ore.** Segnala una nuova versione
  una sola volta, con **Aggiorna ora** o **Ricordamelo domani**, e non installa
  mai da solo. Segue il canale scelto, versioni preliminari comprese.

### Modificato
- **Interruttori al posto delle caselle nelle impostazioni**, disegnati come su
  iOS: grigi quando sono spenti, del colore dell'applicazione quando sono accesi,
  con un pulsante che scorre.
...
### Aggiunto
- **Il pacchetto macOS è firmato e autenticato da Apple.** Finora portava una
  firma ad hoc, che non vale nulla per nessuno tranne la macchina che l'ha
  prodotta: macOS si rifiutava di avviarlo, e riuscirci significava smentire il
  sistema a mano. Ora porta un Developer ID e l'autenticazione di Apple,
  graffata nel pacchetto perché valga anche senza rete. Si avvia con un doppio
  clic, come qualsiasi altra cosa.
- **Sostituzione automatica su macOS.** Il pulsante che dalla 0.4.0 aggiorna la
  versione per Windows fa ora lo stesso qui. La differenza sta in ciò che viene
  sostituito — un intero pacchetto anziché un file — e in ciò che viene
  verificato prima: oltre alla somma SHA-256 pubblicata, la firma della
  versione scaricata, con la stessa domanda che porrebbe Gatekeeper. Ciò che
  macOS non eseguirebbe non viene nemmeno installato.
...
### Aggiunto
- **macOS.** L'applicazione vive ora anche nella barra dei menù: un'icona con i
  limiti segnalati, le stesse finestre, le stesse nove lingue. L'accesso
  integrato finisce nel portachiavi, l'avvio automatico passa per un agente di
  avvio dell'utente, e il token di un'installazione di Claude Code viene letto
  dal portachiavi come prima.

  Il menù lì è nativo: la decisione opposta a quella di Windows, per la stessa
  ragione. Un'icona della barra dei menù apre un menù di sistema, e una finestra
  che ne imitasse uno sarebbe proprio ciò che stonerebbe.

  Distribuita come pacchetto applicativo per Apple silicon, firmato ad hoc e non
...
### Corretto
- **Il riepilogo delle novità non compariva all'uscita da una versione di
  prova.** Quel che era stato eseguito per ultimo veniva annotato con tre
  numeri e senza etichetta: 0.7.1-beta.5 e la 0.7.1 definitiva lasciavano così
  la stessa traccia, il passo tra le due era invisibile e il riepilogo non
  arrivava mai. Ora l'etichetta viene registrata, e arrivare alla versione
