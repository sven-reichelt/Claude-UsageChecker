...
### Adicionado
- **Um guia de utilização, nos nove idiomas.** Percorre tudo o que a aplicação
  faz, janela a janela e com uma imagem de cada uma: instalação, início de sessão,
  ícone, menu, janela de detalhes, avisos, cada definição, atualizações,
  desinstalação e o que fazer quando algo não funciona. As imagens são desenhadas
  em cada idioma a partir de valores de exemplo, pelo que nenhuma contém a conta
  de ninguém.
- **Acerca do Claude UsageChecker → Abrir o guia de utilização** abre-o no idioma
...
- **Um aviso quando um limite chega ao amarelo, ao vermelho ou se esgota.** O
  ícone muda de cor há muito, mas um ícone na área de notificação passa
  facilmente despercebido – no Windows fica muitas vezes na área de excesso,
  onde ninguém o vê. Agora uma janela diz de que limite se trata, até onde
  chegou, o que isso significa e quando será reposto. Usa os limiares já
  definidos para o ícone, e não um par próprio, pelo que o aviso e o ícone nunca
  se contradizem.

  Uma vez por limite e nível até esse limite ser reposto, e memorizado após um
  reinício, para que o arranque automático não repita todas as manhãs o que foi
  dito na véspera. Vários limites ao mesmo tempo partilham uma só janela.
- **É o utilizador que decide o quanto o aviso insiste.** Em **Definições →
  Avisos**: de quais dos três níveis ser avisado, se espera por **OK,
  entendido!** ou se fecha sozinho após um tempo à escolha, e se fica à frente
  de todas as outras janelas. **Mostrar pré-visualização** apresenta-o com
  valores de exemplo – a única forma de ver o aviso de um limite esgotado sem
  esgotar um.
...
### Corrigido
- **O arranque automático podia desaparecer enquanto as definições ainda
  diziam que sim.** A caixa continuava assinalada sobre uma entrada que já não
  existia, e só desmarcar, guardar, assinalar e guardar de novo a repunha. Agora a
  aplicação verifica em cada arranque se a entrada existe e aponta para o programa
  instalado e, caso contrário, repõe-na, seja o que for que a tenha removido. Uma
  entrada desativada no Gestor de Tarefas é respeitada. A única causa encontrada
  foi o próprio conjunto de testes do projeto, que apagava a entrada no computador
  onde corria; também isso já não pode acontecer.
...
- **O macOS é entregue como imagem de disco, e a aplicação instala-se
  ...
  assinatura deles não pode ficar guardada no seu interior: o macOS mantém-na em
  atributos estendidos ao lado. Um zip só os consegue transportar como ficheiros
  acompanhantes, e se voltam a ser atributos é decidido pela ferramenta que o
  descompacta. Quando isso corre mal, 202 dos 221 ficheiros do pacote ficam sem
  assinatura, o reconhecimento já não corresponde a uma assinatura que já não
  vale, e o macOS comunica que não consegue verificar se a aplicação está livre
  de software malicioso — o que soa a acusação e é um atributo de ficheiro
  perdido.

  Uma imagem de disco é montada, não descompactada. Entre a assinatura e o
  sistema que a verifica não está nada. O zip continua anexado à publicação para
  a verificação de atualizações, que o descompacta com `ditto` e assim preserva
  o que importa.

## [0.8.0] – 2026-08-20

### Adicionado
- **macOS.** A aplicação passa a viver também na barra de menús: um ícone com os
  limites indicados, as mesmas janelas, os mesmos nove idiomas. O início de
  sessão próprio vai para o porta-chaves, o arranque automático usa um agente de
  lançamento do utilizador, e o token de uma instalação do Claude Code é lido do
  porta-chaves como antes.

  Aí o menú é nativo: a decisão oposta à do Windows, pela mesma razão. Um ícone
  da barra de menús abre um menú do sistema, e uma janela a imitá-lo seria
  precisamente o que destoaria.

  Entregue como pacote de aplicação para Apple silicon, assinado ad hoc e não por
