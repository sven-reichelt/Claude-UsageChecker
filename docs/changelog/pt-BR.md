...
### Adicionado
- **O plano, onde ele pertence.** O menu o mostra abaixo do uso, a janela de
  detalhes abaixo do horário e da origem, as configurações abaixo dos dois
  logins – por exemplo, "Claude Max 5×". Ele é lido com o mesmo token dos
  números, então é sempre o plano da conta a que eles se referem, e não é
  consultado de novo a cada atualização. Da resposta do servidor, só o plano é
  lido – não o nome nem o endereço de e-mail que vêm junto. Até agora só o Max 5×
  foi visto; Pro, Max 20×, Team e Enterprise devem aparecer da mesma forma, e o
  inesperado é mostrado como chega em vez de ser escondido.
...
### Adicionado
- **O pacote do macOS está assinado e reconhecido pela Apple.** Até agora ele
  ...
  Apple, grampeado dentro do pacote para valer também sem conexão. Ele abre com
  um clique duplo, como qualquer outra coisa.
- **Substituição automática no macOS.** O botão que atualiza a versão do
  ...
  substituído — um pacote inteiro em vez de um arquivo — e no que é verificado
  antes: além da soma SHA-256 publicada, a assinatura da versão baixada, com a
  mesma pergunta que o Gatekeeper faria. O que o macOS não executaria também
  não é instalado.
...
### Adicionado
- **macOS.** O aplicativo agora também mora na barra de menus: um ícone com os
  limites informados, as mesmas janelas, os mesmos nove idiomas. O login
  próprio vai para as chaves, a inicialização automática usa um agente de
  lançamento do usuário, e o token de uma instalação do Claude Code é lido das
  chaves como antes.

  Lá o menu é nativo: a decisão oposta à do Windows, pelo mesmo motivo. Um ícone
  da barra de menus abre um menu do sistema, e uma janela imitando um seria
  justamente o que destoaria.

  Entregue como pacote de aplicativo para Apple silicon, assinado ad hoc e não
...
- **O resumo das novidades não aparecia ao sair de uma versão de teste.** O que
  rodou por último era anotado com três números e sem rótulo, de modo que a
  0.7.1-beta.5 e a 0.7.1 final deixavam o mesmo rastro: o passo entre elas era
  invisível e o resumo nunca vinha. Agora o rótulo é registrado, e chegar à
...
- **O botão de atualizar procura de passagem uma versão nova.** Um clique em
  vez de dois. Pode ser desativado nas configurações, para quem prefere não
  usar a rede mais do que o necessário.
- **As configurações ficam em duas colunas.** Em uma só, a janela ficava alta e
  estreita — na tela de um notebook era preciso rolar por ajustes que cabem
  confortavelmente lado a lado.
...
### Alterado
- **O menu da área de notificação agora é desenhado pelo próprio aplicativo.**
  O Windows desenha menus de contexto com a fonte do sistema e sem moldura
  própria; ao lado das janelas deste aplicativo parecia outro programa. Agora
  tem a mesma moldura, a mesma fonte e os mesmos espaçamentos.
- **O menu indica a versão.** A entrada agora diz «Sobre o Claude
  UsageChecker 0.7.0 …». É a primeira coisa que se pergunta a quem relata um
  problema.
