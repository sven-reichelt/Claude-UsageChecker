...
`Content=` attributes — each window sets its labels in an `ApplyTexts()` method,
...
that the placeholders (`{0}`, `{1}`) match across languages — a `{2}` in a text
...
1. **Never refresh or write back foreign tokens.** Access to Claude Code's
   `.credentials.json` is strictly read-only. Its refresh tokens rotate —
...
it claimed — including one written to guard version comparison that passed just
...
Small and self-contained, please. For anything larger, open an issue first —
